...
function ManageFuelUsage(vehicle)
	if not Entity(vehicle).state.fuel then
		SetFuel(vehicle, math.random(40, 60))
	end

	if IsVehicleEngineOn(vehicle) then
		SetFuel(vehicle, GetFuel(vehicle) - Config.FuelUsage[Round(GetVehicleCurrentRpm(vehicle), 1)] * (Config.Classes[GetVehicleClass(vehicle)] or 1.0) / (10*1.5))
	end
end
	...
	while true do
		...
		if IsPedInAnyVehicle(ped) then
			...
			if not inBlacklisted and GetPedInVehicleSeat(vehicle, -1) == ped then
				local ignition = Entity(vehicle).state.ignition
				
				if ignition and ignition == 2 then
